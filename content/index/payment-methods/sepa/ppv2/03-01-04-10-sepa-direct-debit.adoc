...
[cols="35h,65"]
|===
| URI (Endpoint)             | ``\https://{pp-test-endpoint}``
...
| merchant-account-id m| value | String | Mandatory | 36 | A unique identifier assigned to every _Merchant Account_ (by {payment-provider-name}).
...
.2+| requested-amount m| value | Numeric | Mandatory | 11 | The full amount that is requested/contested in a transaction. 2 decimal
places allowed.
...
| payment-method      m| name     | String | Optional | 15 | The name of the payment method used for the transaction. Set this value to ``sepadirectdebit``.
...
2+| merchant-account-id m| value | String | A unique identifier assigned to every _Merchant Account_ (by {payment-provider-name}).
...
2+|bank-account m| iban          | String | The International Bank Account Number (IBAN).
...
2.2+| requested-amount m| currency | String | The currency of the requested/contested transaction amount. For _SEPA Direct Debit_ payments, the currency must be set to ``EUR``. +
Format: 3-character abbreviation according to ISO 4217.
...
.3+| statuses .3+| status m| description | String | The description of the transaction status message.
...
2.2+| account-holder m|  first-name    | String | The first name of the account holder.
...
2.2+| mandate m| mandate-id  | String | The ID of the Direct Debit mandate. You must generate this value yourself.
...
Format: 3-character abbreviation according to ISO 4217.
| payment-method      m| name     | String | Optional | 15 | The name of the payment method used. Set this value to ``sepadirectdebit``.
...
allowed.
3+| provider-transaction-reference-id | String | An identifier used to match and reference all transactions belonging to a single Direct Debit payment lifecycle.
2.2+| periodic m| periodic-type | String | Indicates if (and how) payment occurs more than once.
