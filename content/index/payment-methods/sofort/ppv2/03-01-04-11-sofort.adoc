...
| merchant-account-id m| value | String | Mandatory | 36 | A unique identifier assigned to every merchant account (by {payment-provider-name}).
...
.2+| requested-amount m| value | Numeric | Mandatory | 18 | The full amount that is requested/contested in a transaction. 2 decimal
places allowed.
...
| payment-method      m| name     | String | Mandatory | 15 | The name of the payment method used. Set this value to ``sofortbanking``.
...
purposes.
| payment-method m| name | String | The name of the payment method used.
...
.3+| status m| code | String | Status code of the status message.
...
.2+| requested-amount m| currency | String | The currency of the requested/contested transaction amount. For
