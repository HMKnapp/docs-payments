...
|===
| Countries
| Contact <<ContactUs, Merchant Support>> for Countries.
| Currencies 
| GBP
|===
...
| debit 
| receive funds from a consumer's account to ship the
merchandise 
| <<API_PaybyBankapp_Samples_debit, debit>> samples.

| refund-debit 
|refund an order, because consumer has returned
merchandise 
|
...
| Contact <<ContactUs, merchant support>> for complete test credentials.
...
. Consumers select products/services on your website. (This can
...
. You redirect consumers to a result page.
...
. You send refund request to Wirecard.
. _{payment-gateway-abbr}_ requests refund account details of the consumers
(transaction type ``refund-request``).
. _{payment-gateway-abbr}_ initiates refund using wiretransfer method (takes several
days).
...
The following elements are **M**andatory or **O**ptional for a request/response/notification. If the respective cell is empty, the field is disregarded or not sent.
...
|transaction-type                                        |M |M |M |String    |22 |This is the type for a transaction. Must be ``refund-request``.
...
When the refund is finished, you receive a refund-debit
notification. This notification is sent as Base64 Encoded XML message, unless
you have a different configuration.
