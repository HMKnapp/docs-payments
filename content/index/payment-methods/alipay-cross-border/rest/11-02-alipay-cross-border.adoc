...
[%autowidth.stretch,stripes=none]
|===
h|URL (Endpoint) 2+|``{rest-api-test-endpoint}``
...
The business type description of your business is stored in the MCC code of your merchant configuration. For other business types than the ones described above, {payment-gateway} adds this MCC code description to your request. +
