...
<<PPv2_AlipayCrossborder_TransactionType_debit, debit>>
...
.2+|requested-amount m|value |Numeric a|The full amount that is requested/contested in a transaction. 2 decimal digits allowed.
...
|payment-method m|name |String |The name of the payment method used. Set this value to ``alipay-xborder``.
...
.3+|account-holder m|first-name |String |The first name of the account holder.
...
is sent to the configured redirection URL. For details on redirection targets after payment and transaction status notifications, see
<<PPSolutions_PPv2_ConfigureRedirects, Configuring Redirects and IPNs for {payment-page-v2-abbr}>>.
...
For _Alipay Cross-border_ payments, the currency must be one of the following: ``AUD``, ``CAD``, ``CHF``, ``DKK``, ``EUR``, ``GBP``, ``HKD``, ``JPY``, ``KRW``, ``NOK``, ``NZD``, ``SEK``, ``SGD``, ``THB``, ``USD``.
m|value |Numeric |The full amount that is requested/contested in a transaction. 2 decimal digits allowed. +
Use . (decimal point) as the separator.
...
|merchant-account-id m|value |String |A unique identifier assigned to every merchant account (by {payment-provider-name}). You receive a unique merchant account ID for each payment method.
...
.3+|account-holder m|first-name |String |The first name of the consumer.
...
post-processing operation - such as a refund of one of your debit transactions
