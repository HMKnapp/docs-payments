...
[#WPP_eps_Banks]
=== Banks Participating in _eps-Überweisung_
...
.2+|bank-account 2+m|bic |String |Optional |8 or 11 a|The bank identifier code (BIC)
of the account owner (consumer).

Allowed characters and format: ([``a-zA-Z``]{4}``[a-zA-Z``]{2}[``a-zA-Z0-9``]{2}([``a-zA-Z0-9``]{3})

2+|iban |String |Optional |34 a|The **I**nternational **B**ank **A**ccount
**N**umber of the account owner (consumer).

Allowed characters and format: [``a-zA-Z``]{2}[``0-9``]{2}[``a-zA-Z0-9``]{4}[``0-9``]{7}([``a-zA-Z0-9``]?){0,16}

.2+|account-holder 2+m|first-name |String |Optional |32 |The first name of the
account holder.
...
debit on the consumer's account to a specific purchase in your shop.
...
.3+|status 2+m|code |String |Status code of the status message.
...
.2+|requested-amount 2+m|currency |String |The currency of the
...
used.
.2+|bank-account 2+m|iban |String |The international bank account number (IBAN)
